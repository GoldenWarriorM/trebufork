...
public class WallpaperOffsetInterpolator {
    ...
    private final OffsetHandler mHandler;
    ...
    public WallpaperOffsetInterpolator(Workspace<?> workspace) {
        ...
        // trebufork: pass root view ref so the handler can update our wallpaper bg.
        mHandler.setRootView(workspace.getRootView());
    }
    ...
    private static class OffsetHandler extends Handler {
        ...
        // trebufork: weak ref to workspace root view for updating our self-contained
        // wallpaper background when the offset changes.
        ...
        private void setOffsetSafely(IBinder token) {
            ...
            // trebufork: also update our self-contained wallpaper background view,
            // since we no longer rely on WM's wallpaper surface for rendering.
            // This handler runs on UI_HELPER_EXECUTOR, so post to main thread for View ops.
            if (mRootView != null) {
                final float offset = mCurrentOffset;
                mRootView.post(() -> {
                    com.android.launcher3.views.WallpaperBackgroundView wpBg =
                            mRootView.findViewById(
                                    com.android.launcher3.R.id.wallpaper_background);
                    if (wpBg != null) {
                        // WM offset is [0..1], our view expects [-1..1]
                        wpBg.setOffsetX(offset * 2f - 1f);
                    }
                });
            }
        }
