...
public class ClipIconView extends View implements ClipPathView {
    ...
    public void update(RectF rect, float progress, float shapeProgressStart, float cornerRadius,
        ...
        // trebufork: translate relative to the container's ACTUAL laid-out position, not the
        // margins. When the tracked icon's bounds arrive late (scrollable-home close), the
        // margin can be updated while the layout position still lags at the old value; using
        // getLeft()/getTop() keeps the translation consistent so the flying icon always lands
        // on the spring target instead of jumping to the layout position (e.g. the screen edge).
        float dX = rect.left - container.getLeft();
        float dY = rect.top - container.getTop();
