...
public class LauncherWidgetHolder {
    ...
    // trebufork: callbacks run (on the main thread) once the host starts listening for widget
    // updates. Used by the scrollable home to re-create widget views that were bound before
    // listening started as pending placeholders (they carry no LauncherAppWidgetInfo tag, so the
    // stock reInflate() path leaves them stuck as "loading" placeholders).
    private final List<Runnable> mOnListeningStartedCallbacks = new ArrayList<>();
    ...
    /**
     * trebufork: registers a callback invoked (on the main thread) once the widget host starts
     * listening for widget updates. If the host is already listening, the callback runs
     * immediately. Callbacks are one-shot: they fire when this holder transitions into the
     * listening state, which is the moment views bound earlier as pending placeholders can be
     * re-created as real host views.
     */
    public void addOnListeningStartedCallback(@NonNull Runnable callback) {
        MAIN_EXECUTOR.execute(() -> {
            if (isListening()) {
                callback.run();
            } else {
                mOnListeningStartedCallbacks.add(callback);
            }
        });
    }

    private void runListeningStartedCallbacks() {
        if (mOnListeningStartedCallbacks.isEmpty()) {
            return;
        }
        List<Runnable> callbacks = new ArrayList<>(mOnListeningStartedCallbacks);
        mOnListeningStartedCallbacks.clear();
