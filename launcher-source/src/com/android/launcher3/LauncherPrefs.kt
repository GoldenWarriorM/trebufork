...
constructor(@ApplicationContext private val encryptedContext: Context) {
    ...
    companion object {
        ...
        @JvmField val SCROLLABLE_HOME = backedUpItem("pref_scrollable_home", false)
        @JvmField val SCROLLABLE_HIDE_LABELS = backedUpItem("pref_scrollable_hide_labels", false)
        // trebufork: show the app search bar on the desktop (not just in the apps list).
        @JvmField val SCROLLABLE_DESKTOP_SEARCH = backedUpItem("pref_scrollable_desktop_search", false)
        ...
        // the right screen edge in dp, block height (0..100 percent of the available height),
        // and the left empty strip in dp (0 = disabled) reserved for left-hand alphabet swipes,
        // and the right-alphabet touch strip width in dp. Values are floats.
        @JvmField val SIDEBAR_POSITION = backedUpItem("pref_sidebar_position", 100f)
        @JvmField val SIDEBAR_EDGE_INSET = backedUpItem("pref_sidebar_edge_inset", 50f)
        @JvmField val SIDEBAR_MARGIN_END = backedUpItem("pref_sidebar_margin_end", 10f)
        @JvmField val SIDEBAR_LEFT_INSET = backedUpItem("pref_sidebar_left_inset", 32f)
        @JvmField val SIDEBAR_HEIGHT = backedUpItem("pref_sidebar_height", 75f)
        @JvmField val SIDEBAR_TOUCH_WIDTH = backedUpItem("pref_sidebar_touch_width", 64f * 2f / 3f)
        // trebufork: top empty strip of the scrollable home (desktop + apps list) and the
        // sidebar, as a percent of the screen height (20 = 20%). Shared by the list headers
        // (ScrollableAppsView) and the alphabet block (ScrollableAppsSidebar). Not exposed in
        // Settings; tune via adb by editing com.android.launcher3.prefs.xml, e.g.
        //   sed -i 's|<float name="pref_scrollable_top_inset" value="20.0"|<float name="pref_scrollable_top_inset" value="30.0"|' .../com.android.launcher3.prefs.xml
        @JvmField val SCROLLABLE_TOP_INSET = backedUpItem("pref_scrollable_top_inset", 20f)
        ...
        // trebufork: master launcher animation speed, as an integer percent (50..200, 100 = 1x
        // default). Stored as a string because the Settings ListPreference writes strings.
        @JvmField val ANIMATION_SPEED = backedUpItem("pref_animation_speed", "100")
