...
import com.android.launcher3.views.WallpaperBackgroundView;
...
public class BaseDepthController {
    ...
    /**
     * trebufork: the scrollable home (single-column apps list) replaces the paged workspace.
     * While it is active no depth blur is applied at all: the dense list looks like a mess when
     * blurred behind the recents live tile / tasks, and it is already faded out once the
     * recents/overview state engages, so the blur would only flash during the swipe transition.
     */
    private boolean isScrollableHomeMode() {
        return LauncherPrefs.SCROLLABLE_HOME.get(mLauncher);
    }
    /**
    ...
    public void setHasContentBehindLauncher(boolean hasContentBehindLauncher) {
        if (mHasContentBehindLauncher != hasContentBehindLauncher) {
            mHasContentBehindLauncher = hasContentBehindLauncher;
            // trebufork: re-apply immediately. The surface opaqueness depends on this flag, so
            // leaving recents (or entering it) must refresh it right away; otherwise the launcher
            // surface can stay opaque and the wallpaper is hidden behind a black background until
            // some unrelated event happens to re-run the depth controller.
            applyDepthAndBlur();
        }
    }
    ...
    private void applyDepthAndBlur(@Nullable SurfaceTransaction surfaceTransaction,
        ...
        float depth = mDepth;
        // trebufork: update our self-contained wallpaper background view.
        View rootView = mLauncher.getRootView();
        if (rootView != null) {
            WallpaperBackgroundView wpBg = rootView.findViewById(R.id.wallpaper_background);
            if (wpBg != null) {
                wpBg.setZoom(depth);
            }
        }
        ...
        int newBlur = mCrossWindowBlursEnabled && !hasOpaqueBg && !mPauseBlurs
                && !isScrollableHomeMode() ? (int) (blurAmount * mMaxBlurRadius) : 0;
    ...
    public boolean blurWorkspaceDepthTargets() {
        // trebufork: the scrollable home is never blurred (see isScrollableHomeMode).
        if (!Flags.allAppsBlur() || isScrollableHomeMode()) {
            return false;
        }
