...
class ScalingWorkspaceRevealAnim(
    ...
    init {
        ...
        // trebufork: with the scrollable single-column home enabled the workspace/hotseat are
        // hidden (GONE) by Launcher and must stay hidden -- animating their alpha here would
        // re-show them (AlphaUpdateListener flips the view to VISIBLE), leaking the dock/QSB
        // through the transparent scrollable list.
        val isScrollableHome = LauncherPrefs.SCROLLABLE_HOME.get(launcher)
        ...
        if (playAlphaReveal && !isScrollableHome) {
        ...
        // trebufork: in scrollable-home mode the tracked icon lives in the ScrollableAppsView,
        // which is a sibling of the workspace and is NOT scaled by this reveal. Re-targeting the
        // spring from the (stale, cached) workspace target here would fight the live scroll
        // tracking done by FloatingIconView.updatePosition() (the icon appears to move in the
        // wrong direction / not follow the finger). Skip it and let the scroll listener drive the
        // target instead.
        if (!isScrollableHome) {
            ...
            animation.addOnFrameListener {
                ...
                // Then we scale back up around the center of the current position. This is because
                // the icon animation behaves poorly if it is given a target that is smaller than
        ...
        animation.addListener(
            AnimatorListeners.forEndCallback(
                Runnable {
                    ...
                    // trebufork: in scrollable-home mode the hotseat is hidden (GONE) by
                    // Launcher and the reveal above never touched it, so leave it hidden.
                    if (isScrollableHome) {
                        hotseat.visibility = View.GONE
                    }
                    if (!isScrollableHome && (!hotseat.isVisible || !workspace.isVisible)) {
                        Log.e(
                            TAG,
