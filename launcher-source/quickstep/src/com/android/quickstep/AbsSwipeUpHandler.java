...
public abstract class AbsSwipeUpHandler<
    ...
    // trebufork: the close-to-icon spring of a PREVIOUS gesture may still be flying the app window
    // to its home icon when a NEW gesture opens recents (scrollable home keeps the spring alive to
    // track the icon during list scroll). Track it here so the recents-open gesture can cancel it
    // (kill the flying icon) instead of letting the two animations overlap.
    @Nullable private static RectFSpringAnim sFlyingIconSpring;
    ...
    public AbsSwipeUpHandler(Context context,
        ...
        mInputConsumerProxy =
            ...
            // trebufork: in scrollable home, a touch that starts a scroll during the
            // close-to-icon animation (end target HOME) must not cut the running window spring
            // short — it keeps tracking the icon (see FloatingIconView's scroll listener and
            // onTargetPositionChanged). For other end targets (recents, quick-switch) keep the
    ...
    public void onRecentsAnimationStart(RecentsAnimationController controller,
        ...
        // trebufork: hide the fresh recents leash IMMEDIATELY as the very first surface op of
        // the gesture, before anything can present it. In the reverse close of an in-progress
        // launch, WM finishes the merged open transition (#44) and reparents the app under a
        // brand-new recents leash (#45) that is created at identity = full screen. There is a
        // window (merge -> onRecentsAnimationStart) where that full-screen leash is live but
        // the launcher does not yet own it, and the old `hideHandoffSurfaceOnCancel` (static,
        // reading the stale merged-transition leash that WM already released = invalid, see
        // logcat "skipped, realSurface=invalid" on EVERY reverse close) never runs - so the
        // full-screen frame flashes. The ONLY surface that survives the gesture is the one in
        // the recents transition info, which we obtain here. Hiding it right away (before
        // reapplyFrozenTransformToRealSurface/applyFrozenToRecentsLeash put the small frozen
        // state back) guarantees no frame between the reparent and the spring can show the
        // window full-screen.
        //
        // trebufork: we deliberately hide ONLY for the true handoff (mActive) case, never for
        // the deferred-close path (mDeferredClose without mActive). Deferred reverse close holds
        // the merged CLOSE transition until the in-progress open animation finishes, at which
        // point the window is already fully open and WM delegates a normal close-from-fullscreen
        // - the guard's own comment says there is no identity-frame flash to hide there. If we
        // hid the leash here in that path, nothing would restore it: the frozen-transform
        // re-apply below is guarded by `if (mActive ...)`, so for mDeferredClose-alone the leash
        // would stay hidden (alpha 0, zero crop) until the close spring's first frame - leaving
        // a pure black full-screen frame instead of the small window.
        if (QuickstepTransitionManager.LaunchHandoff.mActive) {
            hideHandoffRecentsLeashNow();
        }

        // The handle the merge guard stashed came from the merged transition info, which the
        // merge handler releases right after - so re-stash the real surface from the recents
        // transition info, which stays alive through the gesture.
        logHandoff("onRecentsAnimationStart: mActive="
                + QuickstepTransitionManager.LaunchHandoff.mActive
                + " mDeferredClose=" + QuickstepTransitionManager.LaunchHandoff.mDeferredClose
                + " mTaskId=" + QuickstepTransitionManager.LaunchHandoff.mTaskId
                + " mAlpha=" + QuickstepTransitionManager.LaunchHandoff.mAlpha
                + " info=" + (transitionInfo != null ? "present" : "NULL")
                + " appCount=" + (mRemoteTargetHandles != null ? mRemoteTargetHandles.length : -1));
        // trebufork: run the frozen re-apply for BOTH the true handoff (mActive) and the
        // deferred-close path (mDeferredClose). In the deferred reverse close, the merged OPEN
        // transition finishes and WM wipes the real surface transform to identity, while the app
        // is about to become a recents task. If we do not re-apply the (≈identity for a fully
        // open window) frozen transform here, the real surface is left without its state at the
        // recents-animation start and the workspace has already left for Background - producing a
        // single full-screen frame of just the wallpaper (the user's "all elements disappear for
        // one frame" at opening recents). Re-applying keeps the window composited through the
        // boundary; the close spring or the recents card takes it over on its first frame.
        if ((QuickstepTransitionManager.LaunchHandoff.mActive
                || QuickstepTransitionManager.LaunchHandoff.mDeferredClose)
                && transitionInfo != null) {
            QuickstepTransitionManager.LaunchHandoff.mRealSurface =
                    findRealSurfaceForTask(transitionInfo,
                            QuickstepTransitionManager.LaunchHandoff.mTaskId);
            SurfaceControl rs = QuickstepTransitionManager.LaunchHandoff.mRealSurface;
            logHandoff("onRecentsAnimationStart: re-stashed real surface valid="
                    + (rs != null && rs.isValid()));
            // trebufork: the frozen window state applied to the real task surface at the close
            // merge gets wiped by WM's finish transaction for the merged open transition
            // (Transition#buildFinishTransaction resets the surface transform to identity when
            // the open transition finishes). The recents transition's start transaction (which
            // reparents the real surface under a fresh leash) is already applied by the time
            // onAnimationStart fires, so re-applying the frozen state now restores the window to
            // its opening position with no race - closing the full-screen flash that would
            // otherwise show until the close spring takes over.
            reapplyFrozenTransformToRealSurface();
        }
    ...
    /**
     * trebufork: synchronously hides every fresh recents app leash for the handoff task, so the
     * reparented full-screen leash (#45) can never present a full-screen frame before the close
     * spring drives it back down. Called as the FIRST surface op in onRecentsAnimationStart
     * (after assignTargetsForSplitScreen exposes the surviving leashes). The old static
     * QuickstepTransitionManager.hideHandoffSurfaceOnCancel() cannot do this: it runs in
     * onAnimationCancelled (before onRecentsAnimationStart) and holds the OLD merged-transition
     * leash, which WM has already released - every reverse close logs "skipped,
     * realSurface=invalid". Hiding the live recents leash here instead is the reliable path.
     */
    private void hideHandoffRecentsLeashNow() {
        if (mRemoteTargetHandles == null) {
            logHandoff("hideHandoffRecentsLeashNow: skipped, no remote handles");
            return;
        }
        ...
        for (RemoteTargetHandle handle : mRemoteTargetHandles) {
            ...
            for (RemoteAnimationTarget app : targets.unfilteredApps) {
                ...
                if (crop.isEmpty()) {
                    t.setAlpha(app.leash, 0f);
                } else {
                    Rect zero = new Rect(crop.left, crop.top, crop.left, crop.top);
                    t.setAlpha(app.leash, 0f);
                    t.setWindowCrop(app.leash, zero);
                }
                any = true;
            }
        }
        HandoffTrace.applyAndTrace("hideHandoffRecentsLeashNow", t);
        t.close();
        logHandoff("hideHandoffRecentsLeashNow: taskId=" + handoffTaskId + " hidden=" + any);
    }

    /**
     * trebufork: re-applies the frozen window state captured at the close merge to the app's
     * real task surface. Called from onRecentsAnimationStart because WM's finish transaction
     * for the merged open transition resets the surface transform to identity, wiping the state
     * applied at the merge. The recents transition's reparent transaction is already applied at
     * this point, so the frozen matrix/crop land on the real surface under the leash the gesture
     * model drives (visual = model x frozen, matching what the user saw at the merge).
     *
     * trebufork: for an EARLY cancel (mAlpha < 0.5) the merge deliberately hid the surface
     * (setAlpha 0 + zero crop). Re-applying the frozen small matrix here while also setting a
     * non-zero alpha would UN-hide that surface and re-attach the semi-transparent window we
     * specifically hid - leaving the window visible through the spring takeover. So for the
     * early-cancel case re-apply as HIDDEN (alpha 0, zero crop), consistent with the merge, and
     * let the close spring's first frame re-reveal it. Only a fully-open window (alpha >= 0.5)
     * is re-applied with its real small transform.
     */
    private void reapplyFrozenTransformToRealSurface() {
        ...
        if (realSurface == null || !realSurface.isValid()
                || crop.isEmpty() || visual.isEmpty()) {
            logHandoff("reapplyFrozenTransformToRealSurface: skipped, surface="
                    + (realSurface == null ? "null"
                            : (realSurface.isValid() ? "valid" : "invalid"))
                    + " cropEmpty=" + crop.isEmpty() + " visualEmpty=" + visual.isEmpty());
            return;
        }
        float handoffAlpha = QuickstepTransitionManager.LaunchHandoff.mAlpha;
        boolean earlyCancel = handoffAlpha < 0.5f;
        Matrix frozenMatrix = frozenMatrixFromHandoff();
        SurfaceControl.Transaction t = new SurfaceControl.Transaction();
        if (earlyCancel) {
            // Match the merge: keep the surface hidden (alpha 0, zero-size crop) so the
            // full-screen/identity frame WM queues for the fresh leash can never render, and
            // so the semi-transparent in-opening window is not re-attached while the spring
            // jump-starts from the icon.
            t.setAlpha(realSurface, 0f);
            Rect zero = new Rect(crop.left, crop.top, crop.left, crop.top);
            t.setWindowCrop(realSurface, zero);
        } else {
            t.setMatrix(realSurface, frozenMatrix, new float[9]);
            t.setWindowCrop(realSurface, crop);
            t.setAlpha(realSurface, handoffAlpha);
        }
        HandoffTrace.applyAndTrace("reapplyFrozenTransformToRealSurface(realSurface)", t);
        t.close();
        logHandoff("reapplyFrozenTransformToRealSurface: crop=" + crop
                + " visual=" + visual + " alpha=" + handoffAlpha
                + " earlyCancel=" + earlyCancel);

        // trebufork: the freshly-reparented recents leash is shown at identity (full screen)
        // until the close spring's first frame lands its small crop/matrix. While that lands
        // there is a gap (up to a frame or two) where the full-screen real surface shows through
        // under a not-yet-small leash, which the user sees as the "full screen flash". Prevent it
        // by applying the same frozen small transform to the recents app leash now, so both the
        // leash and the real surface are small from the very first frame after the swipe.
        applyFrozenToRecentsLeash(frozenMatrix, crop, handoffAlpha);
    }

    /**
     * trebufork: builds the frozen small-window matrix from the captured handoff state. Empty
     * crop/visual rects are already guarded by the caller; scaleX/scaleY are simple ratios taken
     * from the same captured rects so a one-off helper keeps the two call sites (real surface
     * and recents leash) in lock-step.
     */
    private static Matrix frozenMatrixFromHandoff() {
        Rect crop = QuickstepTransitionManager.LaunchHandoff.mCrop;
        RectF visual = QuickstepTransitionManager.LaunchHandoff.mVisualRect;
        ...
        matrix.postTranslate(visual.left - crop.left * scaleX,
    ...
    /**
     * trebufork: single-point verbose logging for the expanded open/close handoff diagnostics.
     * Delegates to {@link HandoffTrace} so every line carries an uptime timestamp plus the
     * current Choreographer frame time, letting the timeline be correlated 1:1 with
     * SurfaceFlinger-presented frames (`dumpsys SurfaceFlinger --latency`, or `adb logcat -s
     * TrebuforkRev`).
     */
    private void logHandoff(String msg) {
        HandoffTrace.log(msg);
    }

    /**
     * trebufork: applies the frozen (small window) transform to the app's recents leash so that
     * the leash renders the window small from the first frame of the close gesture, closing the
     * gap where a freshly-reparented leash would otherwise show the full-screen real surface
     * behind it. Composes with the gesture model's current matrix (model applied on top of the
     * frozen state), matching the on-screen window the user sees.
     */
    private void applyFrozenToRecentsLeash(Matrix frozenMatrix, Rect crop, float handoffAlpha) {
        if (mRemoteTargetHandles == null) {
            return;
        }
        int handoffTaskId = QuickstepTransitionManager.LaunchHandoff.mTaskId;
        boolean isEarlyCancel = handoffAlpha < 0.5f;
        SurfaceControl.Transaction t = new SurfaceControl.Transaction();
        boolean any = false;
        for (RemoteTargetHandle handle : mRemoteTargetHandles) {
            RemoteAnimationTargets targets = handle.getTransformParams().getTargetSet();
            ...
            for (RemoteAnimationTarget app : targets.unfilteredApps) {
                ...
                if (isEarlyCancel) {
                    // The window was still opening (semi-transparent / near the icon) when the
                    // cancel came. WM already applied it in full-screen for the identity frame that
                    // precedes the close spring, so hide the leash outright (alpha 0 and a
                    // zero-size crop) to make sure that captured full-screen frame can never show.
                    // The close spring, which starts from the icon target for this case, is what
                    // re-reveals the (tiny) window a frame later.
                    t.setAlpha(app.leash, 0f);
                    Rect zero = new Rect(crop.left, crop.top, crop.left, crop.top);
                    t.setWindowCrop(app.leash, zero);
                    any = true;
                ...
                Matrix composed = new Matrix(tvs.getCurrentMatrix());
                composed.postConcat(frozenMatrix);
                t.setMatrix(app.leash, composed, new float[9]);
                t.setWindowCrop(app.leash, crop);
                t.setAlpha(app.leash, handoffAlpha);
                any = true;
            }
        }
        HandoffTrace.applyAndTrace("applyFrozenToRecentsLeash", t);
        t.close();
        logHandoff("applyFrozenToRecentsLeash: crop=" + crop
                + " taskId=" + handoffTaskId + " alpha=" + handoffAlpha
                + " hide=" + isEarlyCancel + " applied=" + any);
    }

    /**
     * trebufork: clears the frozen window state from the app's real task surface when the close
     * gesture ends anywhere but home (cancelled, rejected, quick-switch), so the gesture model
     * can drive the window normally again.
     */
    private void resetFrozenRealSurface() {
        // trebufork: the deferred-close flag must not survive the recents animation: if the
        // gesture ended anywhere but a close spring (cancel/reject/quick-switch/recents-open),
        // a later recents animation must not re-hide the app's leash.
        QuickstepTransitionManager.LaunchHandoff.mDeferredClose = false;
        SurfaceControl realSurface = QuickstepTransitionManager.LaunchHandoff.mRealSurface;
        QuickstepTransitionManager.LaunchHandoff.mRealSurface = null;
        if (realSurface == null || !realSurface.isValid()) {
            return;
        }
        SurfaceControl.Transaction t = new SurfaceControl.Transaction();
        t.setMatrix(realSurface, Matrix.IDENTITY_MATRIX, new float[9]);
        t.setWindowCrop(realSurface, null);
        t.setAlpha(realSurface, 1f);
        HandoffTrace.applyAndTrace("resetFrozenRealSurface", t);
        t.close();
        logHandoff("resetFrozenRealSurface (real surface reset to identity)");
    }

    /**
     * trebufork: returns the app's real task surface for the given task from a transition info.
     */
    private static SurfaceControl findRealSurfaceForTask(TransitionInfo info, int taskId) {
        if (info == null || taskId == -1) {
            return null;
        }
    ...
    public void onRecentsAnimationCanceled(HashMap<Integer, ThumbnailData> thumbnailDatas) {
        ...
        // trebufork: the close gesture was cancelled without a home spring - clear the frozen
        // launch state from the app's real task surface so the window is not left stuck small.
        resetFrozenRealSurface();
    ...
    private void handleNormalGestureEnd(
        ...
        // trebufork: safety net — opening recents while a previous gesture's close-to-icon
        // spring is still flying would overlap the fly-to-icon with the recents-open. Normally
        // the spring is already killed on touch-down (see onTouchDownCallback); this catch-all
        ...
        // gesture model can drive the window normally again.
        if (endTarget != HOME && QuickstepTransitionManager.LaunchHandoff.mActive) {
            resetFrozenRealSurface();
        }
    ...
    private void animateToProgressInternal(float start, float end, long duration,
        ...
        if (mGestureState.getEndTarget() == HOME) {
            ...
            if (mIsSwipingPipToHome) {
                ...
                // trebufork: remember this close-to-icon spring so a later recents-open gesture
                // can cancel it (kill the flying icon) instead of overlapping two animations.
                registerFlyingIconSpring(windowAnim);
    ...
    protected AnimationSuccessListener getWindowAnimationToHomeListener() {
        return new AnimationSuccessListener() {
            ...
            public void onAnimationSuccess(Animator animator) {
                ...
                // trebufork: the close spring drove the app window home and it is now
                // collapsed/offscreen. Reset the (frozen-small) real task surface back to identity
                // so the next launch of the app starts clean and full-screen. Safe to call even if
                // the handoff was already deactivated (resetFrozenRealSurface no-ops on null).
                resetFrozenRealSurface();
    ...
    /**
     * trebufork: cancels the close-to-icon spring still flying its window to the home icon from
     * a previous gesture (killing the floating icon), so it cannot overlap a new gesture's
     * animation. No-op when no spring is running.
     */
    ...
    /**
     * trebufork: remembers the close-to-icon spring ({@link #sFlyingIconSpring}) while it is
     * flying to the home icon, and clears it when the spring ends or is cancelled so stale state
     * can never cancel a later gesture's animation.
     */
    private static void registerFlyingIconSpring(RectFSpringAnim[] anims) {
        RectFSpringAnim anim = anims != null && anims.length > 0 ? anims[0] : null;
        if (anim == null) {
            return;
        }
        sFlyingIconSpring = anim;
    ...
    private void invalidateHandler() {
        ...
        mInputConsumerProxy.unregisterOnTouchDownCallback();
        // trebufork: in scrollable home, do not fast-forward the closing-window spring when the
        // handler is invalidated with end target HOME (the close-to-icon case — a scroll switches
        // the input consumer away from the swipe-up gesture). Let it keep tracking the icon while
        // the list scrolls; the ReleaseCheck keeps the remote-animation targets alive until the
