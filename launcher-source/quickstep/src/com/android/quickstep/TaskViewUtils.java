...
public final class TaskViewUtils {
    ...
    void createRecentsWindowAnimator(
        ...
        for (RemoteTargetHandle targetHandle : remoteTargetHandles) {
            ...
            out.addOnFrameCallback(() -> {
                ...
                // (swipe up while the app is still opening) can start the close spring from
                // exactly this position. Once the close merge has armed the handoff, stop
                // updating it: the captured state is what the close transition's leash was
                // set to, so the spring must start from the same values.
                if (!QuickstepTransitionManager.LaunchHandoff.mActive
                        && remoteTargetHandles.length > 0) {
                    TaskViewSimulator tvs = remoteTargetHandles[0].getTaskViewSimulator();
                    try {
                        tvs.getCurrentCropRect().round(
                                QuickstepTransitionManager.LaunchHandoff.mCrop);
                        QuickstepTransitionManager.LaunchHandoff.mVisualRect.set(
                                tvs.getCurrentRect());
                        QuickstepTransitionManager.LaunchHandoff.mAlpha = 1f;
                    } catch (NullPointerException e) {
                        // The simulator's device profile is not set yet on the very first
                        // frames of the launch animation; skip the capture this frame.
                    }
                }
            });
    ...
    public static void composeRecentsLaunchAnimator(@NonNull AnimatorSet anim, @NonNull View v,
        ...
        if (launcherClosing) {
            ...
            // trebufork: master launcher animation speed - scale the recents-view part of the
            // reverse launch too. Its children are attached synchronously at creation, so scaling
            // the subtree right here guarantees it runs at the same speed as the window part
            // (which LauncherAnimationRunner scales later), preventing the "double animation"
            // (slowed window + stock-speed recents cards) at non-1x speeds.
            com.android.launcher3.AnimationSpeed.applyDurationScale(
                    recentsView.getContext(), launcherAnim);
        ...
        // trebufork: master launcher animation speed - scale the composed reverse-launch tree so
        // every child (window part AND recents-view part) honors the chosen speed. The
        // WeakHashMap guard in AnimationSpeed makes this idempotent when the caller
        // (LauncherAnimationRunner.setAnimation) scales the same AnimatorSet again.
        com.android.launcher3.AnimationSpeed.applyDurationScale(recentsView.getContext(), anim);
