...
internal constructor(
    ...
    override suspend fun getThumbnail(task: Task): ThumbnailData? {
        ...
        // trebufork: the cache is the only trusted source of freshness. It is updated on every
        // system-pushed snapshot (RecentsModel.onTaskSnapshotChanged -> updateTaskSnapShot),
        // including the one captured at minimize time. task.thumbnail is deliberately NOT used
        // as a fallback: since TasksRepository.removeTasks keeps the task data across recents
        // visibility changes, task.thumbnail may be a stale copy from a previous recents
        // session. The cache entry for the same task can be missing or invalidated by the time
        // we get here - evicted (phone cache holds only recentsThumbnailCacheSize=3 tasks, and
        // updateIfAlreadyInCache drops pushes for evicted tasks) or invalidated by
        // getAndInvalidateIfModified when the task key's lastActiveTime changed after the app
        // was reopened. In both cases the fresh snapshot the system captured is only reachable
        // through a direct query, so returning task.thumbnail here would pin a stale preview
        // forever. The model still renders the previous thumbnail instantly at bind time, so
        // the async system query below replaces it without any white flash.
