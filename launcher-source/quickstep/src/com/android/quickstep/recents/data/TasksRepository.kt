    ...
    private fun removeTasks(tasksToRemove: Set<Int>) {
        ...
        tasks.update { currentTasks ->
            tasksToRemove.forEach { taskId ->
                ...
                // trebufork: keep the task data (thumbnail, icon, title) so a recents re-open
                // right after a close renders the cards instantly instead of flashing the flat
                // background color (BackgroundOnly) while snapshots re-fetch (~40ms of white),
                // and instead of losing the app names. Clearing the icon would also break the
                // titles: TaskIconCache.getIcon() short-circuits when task.icon is already set
                // and returns the CURRENT task.title/titleDescription - which would be null.
                // The data is small (low-res bitmap + icon + two strings per task) and the map
                // is rebuilt from the system list on every forceRefresh, so it cannot grow
                // unboundedly.
                // (No clearing here - the Task stays in the map with its data.)
            }
