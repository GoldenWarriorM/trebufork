...
class TaskContentView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    // Defaults are overwritten in init; vars (not vals) so the lazy animator
    // blocks below can reference them (Kotlin 2.x: "variable must be
    // initialized" for vals captured by earlier-declared lazy initializers).
    private var focusBorderColor = DEFAULT_BORDER_COLOR
    private var hoverBorderColor = DEFAULT_BORDER_COLOR
    ...
    init {
        ...
        // Explicit obtain/recycle instead of `use {}`: Kotlin 2.x does not treat
        // the lambda as definitely assigning the two vals ("variable must be
        // initialized").
        val ta = context.obtainStyledAttributes(attrs, R.styleable.TaskContentView)
        try {
            focusBorderColor =
                ta.getColor(R.styleable.TaskContentView_focusBorderColor, DEFAULT_BORDER_COLOR)
            hoverBorderColor =
                ta.getColor(R.styleable.TaskContentView_hoverBorderColor, DEFAULT_BORDER_COLOR)
        } finally {
            ta.recycle()
        }
    }
