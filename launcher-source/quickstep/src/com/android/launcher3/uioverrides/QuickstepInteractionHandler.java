...
class QuickstepInteractionHandler implements RemoteViews.InteractionHandler,
    ...
    public boolean onInteraction(View view, PendingIntent pendingIntent,
        ...
        // trebufork: honor the widget developer's shared-element open/close animation protocol
        // (RemoteViews.RemoteResponse#addSharedElement). When the provider opted in, the framework
        // already built a scene-transition ActivityOptions and stashed the shared-element bounds
        // into the fill-in intent; keep them as-is so the activity opens from (and closes back
        // into) the widget instead of using the generic Quickstep scale animation.
        if (pendingIntent.isActivity() && options != null && options.first != null
                && options.second != null
                && options.first.hasExtra(RemoteViews.EXTRA_SHARED_ELEMENT_BOUNDS)) {
            options.second.setPendingIntentLaunchFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            options.second.setSplashScreenStyle(SplashScreen.SPLASH_SCREEN_STYLE_SOLID_COLOR);
            options.second.setPendingIntentBackgroundActivityStartMode(
