...
public class PortraitStatesTouchController extends AbstractStateChangeTouchController {
    ...
    protected boolean canInterceptTouch(MotionEvent ev) {
        // trebufork: with the scrollable single-column home enabled, let the list handle the
        // body swipe instead of opening the app drawer. A swipe that starts on the nav-bar edge
        // must still be intercepted so the recents/overview gesture (handled by
        // NoButtonNavbarToOverviewTouchController, which calls this method via super) works.
        if (mLauncher.isInState(NORMAL) && LauncherPrefs.SCROLLABLE_HOME.get(mLauncher)
                && (ev.getEdgeFlags() & EDGE_NAV_BAR) == 0) {
            return false;
        }
