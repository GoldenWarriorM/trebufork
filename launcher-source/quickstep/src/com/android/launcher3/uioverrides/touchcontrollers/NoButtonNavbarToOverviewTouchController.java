...
public class NoButtonNavbarToOverviewTouchController extends PortraitStatesTouchController {
    ...
    public void onDragStart(boolean start, float startDisplacement) {
        // trebufork: this controller only starts a drag for a REAL nav gesture (canInterceptTouch
        // true — on the launcher view path, EDGE_NAV_BAR is reliable and a scrollable-home touch on
        // the list body deliberately returns false so the list scrolls). That is exactly the
        // "launcher-registered swipe": cancel the close-to-icon spring still flying to a home icon
        // from a previous gesture here, so a nav-bar swipe to recents removes the icon, while a
        // plain list scroll (which never reaches onDragStart) keeps the closing animation running.
        AbsSwipeUpHandler.killFlyingIcon();
        ...
        if (mFromState == NORMAL && mToState == HINT_STATE) {
            // trebufork: a light nav-pill swipe closes the alphabet list and lands on the desktop.
            // In scrollable-home mode there is no dimming during this peek — the list simply swaps
            // to the desktop with a quick fade (see ScrollableAppsView#showDesktop).
            mLauncher.showScrollableDesktop();
            if (!LauncherPrefs.SCROLLABLE_HOME.get(mLauncher)) {
