...
public class QuickstepLauncher extends Launcher implements RecentsViewContainer,
    ...
    protected boolean shouldDeferLaunchForRunningTransition() {
        // trebufork: defer launches that arrive while the closing-window spring is still running
        // (scrollable home keeps the launcher touchable during the close-to-icon animation).
        // Launching then would be merged into the still-active recents transition and the open
        // animation would be skipped.
        TaskAnimationManager tam = TaskAnimationManager.REPOSITORY_INSTANCE.get(this)
                .get(getDisplayId());
        boolean running = tam != null && tam.isRecentsAnimationRunning();
        boolean defer = running;
        Log.d(TREBUFORK_TAG, "shouldDeferLaunchForRunningTransition: running=" + running
                + " tam=" + (tam != null) + " state=" + getStateManager().getState());
        if (defer) {
            Log.d(TREBUFORK_TAG, "shouldDeferLaunchForRunningTransition: deferring launch"
    ...
    protected void deferLaunchForRunningTransition(Runnable relaunch) {
        // Finish the running closing-window/recents transition to home before re-issuing the
        // launch. Re-issuing while the recents animation is still active makes WM Shell merge the
        // open transition into the unfinished close transition and the open animation is skipped.
        // We force-finish the recents controller (synchronous binder round-trip, not the remaining
        // spring) so the launch then runs as a fresh transition. The wallpaper is rendered by the
        // self-contained WallpaperBackgroundView, so WM never needs to restore it here.
        TaskAnimationManager tam = TaskAnimationManager.REPOSITORY_INSTANCE.get(this)
                .get(getDisplayId());
        if (tam == null || !tam.isRecentsAnimationRunning()) {
            Log.d(TREBUFORK_TAG, "deferLaunchForRunningTransition: transition already finished, "
                    + "re-issuing immediately");
            relaunch.run();
            return;
        }
        Log.d(TREBUFORK_TAG, "deferLaunchForRunningTransition: force-finishing recents, "
                + "then re-issuing launch");
        tam.finishRunningRecentsAnimation(
                /* toHome= */ true,
                /* forceFinish= */ true,
                /* forceFinishCb= */ relaunch,
                new com.android.quickstep.util.ActiveGestureLog.CompoundString(
                        "QuickstepLauncher.deferLaunchForRunningTransition: "
                                + "finishing running transition before deferred launch"));
    }
    ...
    public TouchController[] createTouchControllers() {
        ...
        list.add(new StatusBarTouchController(this, () -> this.isInState(LauncherState.NORMAL)
                && !shouldBlockScrollableHomeStatusBarSwipe()));
