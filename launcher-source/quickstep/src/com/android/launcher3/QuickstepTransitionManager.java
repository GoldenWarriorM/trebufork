...
public class QuickstepTransitionManager implements OnDeviceProfileChangeListener {
    ...
    /**
     * trebufork: handoff state for a swipe-up close while the launch animation is still running.
     * When the close merge arrives, the guard arms this handoff and delegates the merge to the
     * default handler (which cancels the open animation and finishes the open transition, so the
     * close transition can run its own animation). The gesture flow's close spring then starts
     * from exactly the window position captured here instead of the gesture model's (which
     * assumes the app was already full screen). Both the launch animation and the close spring
     * drive the leash with crop+matrix, so the handoff is seamless.
     */
    public static final class LaunchHandoff {
        /** True from the close merge until the close spring consumes the state (or a timeout). */
        public static boolean mActive = false;
        /**
         * trebufork: true while a close gesture is HELD (deferred) because the open animation is
         * still running, and until the close spring consumes it. The deferred path deliberately
         * does not arm the full handoff (mActive stays false so the close spring starts from the
         * gesture position, not a stale mid-open rect), but the fresh recents leash created when
         * the merge is delegated still needs to be hidden at onRecentsAnimationStart to avoid the
         * full-screen identity frame (the "white flash" after an interrupted launch).
         * AbsSwipeUpHandler checks this flag alongside mActive.
         */
        public static boolean mDeferredClose = false;
        /** Window crop of the opening app target at the moment of the close merge. */
        public static final Rect mCrop = new Rect();
        /** On-screen visual rect of the window (crop mapped through the open matrix). */
        public static final RectF mVisualRect = new RectF();
        /** Leash alpha at the moment of the close merge (the open animation fades the window in). */
        public static float mAlpha = 1f;
        /** The launch animation runner, used to stop the open animation once the close spring
         * takes over. */
        public static LauncherAnimationRunner mRunner;
        /** Task id of the opening app the handoff belongs to (for split-screen filtering). */
        public static int mTaskId = -1;
        /** The app's real task surface (not the transition leash) at the close merge. The frozen
         * window state is applied to it so the window survives the open transition's finish and
         * the recents transition's fresh leashes without snapping to full screen. */
        public static SurfaceControl mRealSurface;
        /** Uptime (ms) when the close handoff was armed (app tap / reverse merge). */
        public static long mArmUptimeMs = -1;
    }

    // trebufork: if the close gesture never produces a spring animation (e.g. it is cancelled
    // right after the merge), clear the handoff so a later normal close does not use stale state.
    private static final long HANDOFF_FALLBACK_MS = 1200;
    private final Runnable mClearHandoffRunnable = () -> {
        LaunchHandoff.mActive = false;
        LaunchHandoff.mDeferredClose = false;
        // The gesture never reached the spring (or the launcher was destroyed); make sure the
        // frozen state is cleared from the app's real surface so the window can't stay stuck.
        resetHandoffRealSurface();
    };

    /**
     * trebufork: clears the frozen window state from the app's real task surface captured at the
     * close merge (see {@link LaunchHandoff#mRealSurface}). Used when the close gesture ends
     * without a home spring (cancelled, rejected, quick-switch) or times out.
     */
    private static void resetHandoffRealSurface() {
        SurfaceControl realSurface = LaunchHandoff.mRealSurface;
        LaunchHandoff.mRealSurface = null;
        ...
        HandoffTrace.applyAndTrace("resetHandoffRealSurface", t);
        t.close();
        HandoffTrace.log("resetHandoffRealSurface: cleared identity on stale real surface");
    }

    /**
     * trebufork: obviate the full-screen identity frame WM presents when it finishes the merged
     * open transition during a reverse close (see logs: WM sends IRemoteAnimationFinishedCallback's
     * onAnimationCancelled -> finish() resets the real task surface to identity, which is exactly
     * the single host frame the user reports). Called from LauncherAnimationRunner's
     * onAnimationCancelled BEFORE finish() so the real surface is left HIDDEN (alpha 0, zero crop)
     * and the window stays invisible until the close spring drives its leash back up - instead of
     * flashing full screen for one frame.
     */
    public static void hideHandoffSurfaceOnCancel() {
        if (!LaunchHandoff.mActive) {
            HandoffTrace.log("hideHandoffSurfaceOnCancel: skipped, mActive=false");
            return;
        }
        SurfaceControl realSurface = LaunchHandoff.mRealSurface;
        if (realSurface == null || !realSurface.isValid()) {
            HandoffTrace.log("hideHandoffSurfaceOnCancel: skipped, realSurface="
                    + (realSurface == null ? "null"
                            : (realSurface.isValid() ? "valid" : "invalid")));
            return;
        }
        Rect crop = LaunchHandoff.mCrop;
        SurfaceControl.Transaction t = new SurfaceControl.Transaction();
        t.setAlpha(realSurface, 0f);
        Rect zero = new Rect(crop.left, crop.top, crop.left, crop.top);
        t.setWindowCrop(realSurface, zero);
        HandoffTrace.applyAndTrace("hideHandoffSurfaceOnCancel", t);
        t.close();
        HandoffTrace.log("hideHandoffSurfaceOnCancel: hid real surface before WM finish reset");
    }

    private void armCloseHandoff() {
        LaunchHandoff.mActive = true;
        LaunchHandoff.mArmUptimeMs = android.os.SystemClock.uptimeMillis();
        mHandler.removeCallbacks(mClearHandoffRunnable);
        mHandler.postDelayed(mClearHandoffRunnable, HANDOFF_FALLBACK_MS);
        HandoffTrace.log("armCloseHandoff: crop=" + LaunchHandoff.mCrop
                + " visualRect=" + LaunchHandoff.mVisualRect
                + " alpha=" + LaunchHandoff.mAlpha);
    }
    ...
    /**
     * trebufork: guards the app-launch remote transition against merges that would cancel the
     * open animation. When the same app is re-launched right after being closed (the scrollable
     * home keeps the launcher touchable while the close-to-icon spring is still running), WM
     * Shell issues both an OPEN and a TO_FRONT transition for the same task. The TO_FRONT merges
     * into the running OPEN's remote transition, and the default merge handler cancels the open
     * animation — the "skipped open animation". Since the OPEN transition already places and
     * stacks the app window, the merged TO_FRONT is redundant: absorb it (discard its changes,
     * finish it immediately) and let the open animation play to its natural end. All other merges
     * (another app opening, a close gesture) keep the default behavior.
     */
    private static class LaunchMergeGuardTransition extends RemoteTransitionStub {

        private static final int INVALID_TASK_ID = -1;

        private final IRemoteTransition mDelegate;
        private final LauncherAnimationRunner mRunner;
        private final Runnable mOnReverseRequested;
        private int mLaunchTaskId = INVALID_TASK_ID;

        LaunchMergeGuardTransition(IRemoteTransition delegate, LauncherAnimationRunner runner,
                Runnable onReverseRequested) {
            mDelegate = delegate;
            mRunner = runner;
            mOnReverseRequested = onReverseRequested;
            LaunchHandoff.mRunner = runner;
        }
        ...
        public void startAnimation(IBinder token, TransitionInfo info,
            ...
            mLaunchTaskId = findAppTaskId(info);
        ...
        public void mergeAnimation(IBinder token, TransitionInfo info,
            ...
            int mergedTaskId = findAppTaskId(info);
            boolean sameLaunchTask = mLaunchTaskId != INVALID_TASK_ID
                    && mergedTaskId == mLaunchTaskId;
            boolean absorb = sameLaunchTask && isOpenOrToFront(info);
            boolean reverse = sameLaunchTask && isClosingMode(info, mergedTaskId);
            HandoffTrace.log("LaunchMergeGuardTransition.mergeAnimation: mergedTask=" + mergedTaskId
                    + " launchTask=" + mLaunchTaskId
                    + " type=" + (info == null ? -1 : info.getType())
                    + " absorb=" + absorb + " reverse=" + reverse);
            if (absorb) {
                // The merged transition only re-stacks/re-opens the very task the open animation
                // is already showing. Discard its changes and finish it immediately so WM Shell
                // can proceed, but do NOT cancel the running open animation.
                t.close();
            ...
            // trebufork: DEFERRED REVERSE CLOSE. When the user swipes to close while the same
            // task's OPEN animation is still running, we must NOT delegate the merge now:
            // delegating cancels the running open transition and WM composes a single full-screen
            // identity frame on the app's real surface (the flash we avoid). Instead we HOLD the
            // merged CLOSE transition (neither delegate nor finish it) and, once the open animation
            // has fully finished, delegate the merge as a normal close-from-fullscreen. At that
            // point the window is already fully open, so the recents close spring takes it back to
            // home with a proper animation and there is no identity-frame flash. We deliberately do
            // NOT applyFrozenTransformToRealSurface/armCloseHandoff here: the open must play out
            // to full screen uninterrupted, and the deferred close must start from full screen (the
            // gesture model), not from a mid-open frozen position.
            if (reverse && mRunner.hasActiveAnimation()) {
                LaunchHandoff.mTaskId = mergedTaskId;
                LaunchHandoff.mDeferredClose = true;
                HandoffTrace.log("merge reverse: DEFERRED (open still animating), holding close"
                        + " taskId=" + mergedTaskId
                        + " handoffAlpha=" + LaunchHandoff.mAlpha
                        + " visual=" + LaunchHandoff.mVisualRect);
                boolean held = mRunner.deferCloseUntilAnimationFinished(() -> {
                    HandoffTrace.log("deferred close: open finished, delegating reverse merge taskId="
                            + mergedTaskId
                            + " visual=" + LaunchHandoff.mVisualRect
                            + " alpha=" + LaunchHandoff.mAlpha);
                    try {
                        mDelegate.mergeAnimation(token, info, t, mergeTarget, finishCallback);
                    } catch (RemoteException e) {
                        HandoffTrace.log("deferred close: delegate threw, forcing finished callback");
                        t.close();
                        if (finishCallback != null) {
                            try {
                                finishCallback.onTransitionFinished(null, null);
                ...
                if (held) {
                    return; // close will be delegated once the open animation finishes
                }
                HandoffTrace.log("merge reverse: open animation already finished, closing now");
            }
            if (reverse) {
                // The user started closing the app while its open animation had ALREADY finished.
                // Normal close: freeze the captured window state onto the app's REAL task surface,
                // arm the close handoff and delegate the merge so the recents gesture produces the
                // close spring from exactly where the window currently is. Since the open animation
                // is not running, delegating cannot cancel it and there is no full-screen flash.
                LaunchHandoff.mTaskId = mergedTaskId;
                HandoffTrace.log("merge reverse: open animation alive="
                        + mRunner.hasActiveAnimation()
                        + " taskId=" + mergedTaskId
                        + " handoffAlpha=" + LaunchHandoff.mAlpha);
                applyFrozenTransformToRealSurface(info, mergedTaskId);
                mOnReverseRequested.run();
                mDelegate.mergeAnimation(token, info, t, mergeTarget, finishCallback);
                return;
            }
            mDelegate.mergeAnimation(token, info, t, mergeTarget, finishCallback);
        ...
        private static boolean isOpenOrToFront(TransitionInfo info) {
            if (info == null) {
                return false;
            }
        ...
        private static boolean isClosingMode(TransitionInfo info, int taskId) {
            if (info == null || taskId == INVALID_TASK_ID) {
                return false;
            }
        ...
        private static int findAppTaskId(TransitionInfo info) {
            if (info == null) {
        ...
        /**
         * trebufork: applies the window state captured at the close merge (see
         * {@link LaunchHandoff}) to the app's real task surface. Unlike the transition leashes
         * (released when the open transition finishes, recreated fresh by the recents
         * transition), the real task surface survives both, so the window keeps showing at its
         * opening position instead of flashing to full screen between the merge and the close
         * spring.
         */
        private static void applyFrozenTransformToRealSurface(
                TransitionInfo info, int taskId) {
            if (LaunchHandoff.mCrop.isEmpty() || LaunchHandoff.mVisualRect.isEmpty()) {
                return;
            }
            for (int i = 0; i < info.getChanges().size(); i++) {
                TransitionInfo.Change change = info.getChanges().get(i);
                ...
                SurfaceControl realSurface = change.getLeash();
                if (realSurface == null || !realSurface.isValid()) {
                    return;
                }
                LaunchHandoff.mRealSurface = realSurface;
                Rect crop = LaunchHandoff.mCrop;
                RectF visual = LaunchHandoff.mVisualRect;
                float alpha = LaunchHandoff.mAlpha;
                SurfaceControl.Transaction t = new SurfaceControl.Transaction();
                if (alpha < 0.5f) {
                    // trebufork: still-opening window -> this is an early cancel. WM is about to
                    // present the window in full-screen (identity) for a single frame before the
                    // close spring kicks in; hide the real surface outright so that full-screen
                    // frame can never show.
                    t.setAlpha(realSurface, 0f);
                    Rect zero = new Rect(crop.left, crop.top, crop.left, crop.top);
                    t.setWindowCrop(realSurface, zero);
                } else {
                    ...
                    t.setMatrix(realSurface, matrix, new float[9]);
                    t.setWindowCrop(realSurface, crop);
                    t.setAlpha(realSurface, alpha);
                }
                HandoffTrace.applyAndTrace("merge.applyFrozenTransformToRealSurface", t);
                t.close();
                HandoffTrace.log("merge.applyFrozenTransformToRealSurface: taskId=" + taskId
                        + " crop=" + crop + " visual=" + visual
                        + " alpha=" + alpha + " hide=" + (alpha < 0.5f));
                return;
            }
        }
    }
    ...
    public ActivityOptionsWrapper getActivityLaunchOptions(View v, ItemInfo itemInfo) {
        ...
        LauncherAnimationRunner appLaunchRunner = createAppLaunchRunner(
                v, onEndCallback);
     ...
     * method.
     */
    private LauncherAnimationRunner createAppLaunchRunner(View v, RunnableList onEndCallback) {
        LaunchHandoff.mActive = false;
        mHandler.removeCallbacks(mClearHandoffRunnable);
    ...
    private IRemoteTransition createAppLaunchRemoteTransition(
            LauncherAnimationRunner appLaunchRunner) {
        IRemoteTransition defaultAppLaunchTransition = appLaunchRunner.toRemoteTransition();
        // trebufork: guard the launch transition so a redundant same-task merge (TO_FRONT created
        // when re-launching the app right after closing it) does not cancel the open animation,
        // and a close merge (swipe up while the open animation is still running) keeps the
        // captured window state so the close spring starts from exactly where the window is
        // instead of from the gesture model's (near) full-screen position.
        IRemoteTransition guardedLaunchTransition = new LaunchMergeGuardTransition(
                defaultAppLaunchTransition, appLaunchRunner, () -> {
                    armCloseHandoff();
                });
        if (!com.android.window.flags.Flags.enableCrossDisplaysAppLaunchTransition()) {
            return guardedLaunchTransition;
        }
    ...
    private void composeIconLaunchAnimator(@NonNull AnimatorSet anim, @NonNull View v,
        ...
        // trebufork: the launch animator is intentionally NOT registered with the state manager
        // (setCurrentAnimation). When the user starts closing the app (swipe up) while the open
        // animation is still running, the gesture flow calls StateManager.goToState() which
        // cancels the current state animation. If the launch animator were registered it would be
        // cancelled right there (snapping the window to full screen) before the launch-merge guard
        // gets a chance to freeze it for the close handoff.
    ...
    private void composeWidgetLaunchAnimator(
            ...
            @NonNull RemoteAnimationTarget[] nonAppTargets,
            boolean launcherClosing) {
        // trebufork: see composeIconLaunchAnimator - the launch animator must not be registered
        // with the state manager so the swipe-up gesture cannot cancel it before it is frozen
        // for the close handoff.
            ...
            public void onUpdate(float percent, boolean initOnly) {
                ...
                for (int i = appTargets.length - 1; i >= 0; i--) {
                    ...
                    if (target.mode == MODE_OPENING) {
                        ...
                        // trebufork: keep the current window state fresh so the close handoff
                        // (swipe up while the open animation is still running) can start the
                        // close spring from exactly this position. Once the close merge has
                        // armed the handoff, stop updating it: the captured state is what the
                        // close transition's leash was set to, so the spring must start from
                        // the same values.
                        if (!LaunchHandoff.mActive) {
                            LaunchHandoff.mCrop.set(crop);
                            LaunchHandoff.mVisualRect.set(crop);
                            matrix.mapRect(LaunchHandoff.mVisualRect);
                            LaunchHandoff.mAlpha = 1f - mIconAlpha.value;
                        }
    ...
    private Animator getBackgroundAnimator() {
        ...
        if (Flags.allAppsBlur()) {
            // trebufork: still play the wallpaper zoom on app open. The close-to-home reveal
            // (ScalingWorkspaceRevealAnim) animates depth too, so without this the open has no
            // reverse zoom and the wallpaper sits at the home scale while the app opens. The
            // self-contained WallpaperBackgroundView follows stateDepth via setZoom, so the zoom
            // is cheap; suppress only the cross-window blur that made us skip this animator.
