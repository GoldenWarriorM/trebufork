...
constructor(
    ...
    private fun sendAccessibilityAnnouncement(shortcutView: View?) {
        if (
            shortcutView == null ||
                // `===` (not `==`): Kotlin 2.x cannot resolve `equals` for a
                // type parameter with both Context and ActivityContext upper
                // bounds ("overload resolution ambiguity"). Identity is
                // equivalent for a null check.
                mTarget === null ||
